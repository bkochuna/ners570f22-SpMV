...
namespace SpMV {
  ...
  SparseMatrix<fp_type> *SparseMatrix_ELL<fp_type>::getFormat(string fmt) {
    ...
    // This is currently commented out to get code complied and tests passed
    // uncomment these lines after all of the different matrix types are merged
  ...
  void SparseMatrix_ELL<fp_type>::computeMatVecProduct(const fp_type x[], fp_type y[]) {
    ...
    This function also assumes a private class variable _col for recording the column indices of elements in each row.
    Assumes size (N x _maxRow) for array This function also assumes a private class variable _val for recording the
    values of elements in each row. Assumes size (N x _maxRow) for array
    */
    if (this->_state != assembled) {
      assembleStorage();
    }
...
#pragma omp parallel for simd schedule(static) collapse(2)
    for (int i = 0; i < this->_nrows; i++) {
      for (int j = 0; j < this->_maxRow; j++) {
        j = j + this->_maxRow * i; // this row assumes that all elements in the columns are stored in contiguous memory
                                   // pointers and allows indexing through one single index
        c = this->J[j];
