...
protected:
  ...
  map<pair<size_t, size_t>, fp_type>
      _buildCoeff; ///< The data structure used for storing non-zero entries
                   ///< during matrix assembly

  /**
   * @brief Convert the assembled matrix data back to the format used for
   * building
   *
   */
  ...
  /**
   * @brief Clear the the unassembled matrix data, should only be called at the
   * end of the matrix assembly
   *
   */
...
template <class fp_type> SparseMatrix<fp_type>::~SparseMatrix() {
  this->_state = undefined;
  assert(this->_state == undefined);
}

template <class fp_type>
void SparseMatrix<fp_type>::setCoefficient(const size_t row, const size_t col,
                                           const fp_type aij) {
  assert(this->_state != undefined);
...
template <class fp_type> void SparseMatrix<fp_type>::getFormat() {
