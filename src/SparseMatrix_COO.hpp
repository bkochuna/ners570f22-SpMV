...
// ==============================================================================
namespace SpMV {
template <class fp_type> class SparseMatrix_COO : public SparseMatrix<fp_type> {
...
public:
  ...
  /**
   * @brief Compute the product of this matrix and a vector in COO format (y =
   * Ax)
   *
...
template <class fp_type> void SparseMatrix_COO<fp_type>::assembleStorage() {
...
template <class fp_type> void SparseMatrix_COO<fp_type>::_unAssemble() {
  assert(this->_state == assembled);
  size_t n = 0;
...
template <class fp_type> void SparseMatrix_COO<fp_type>::~SparseMatrix_COO() {
...
void SparseMatrix_COO<fp_type>::computeMatVecProduct(const fp_type x[],
                                                     fp_type y[]) {
